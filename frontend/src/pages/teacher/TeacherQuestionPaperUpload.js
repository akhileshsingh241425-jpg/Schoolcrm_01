...
import {
  CloudUpload, Description, CheckCircle, HourglassEmpty, ThumbDown,
  Refresh, Close, Add
...
export default function TeacherQuestionPaperUpload() {
  ...
  const statusConfig = {
    ...
    hod_approved: { color: '#3b82f6', label: 'Approved' },
  ...
  return (
    <Box>
      ...
      {/* Select Exam to view papers */}
      <Paper sx={{ p: 2, mb: 3, borderRadius: 3 }}>
        <TextField select size="small" label="Select Exam to view uploads" sx={{ minWidth: 250 }}
          onChange={e => loadPapers(e.target.value)}>
      ...
      {papers.length > 0 ? (
        ...
        <Paper sx={{ p: 4, textAlign: 'center', borderRadius: 3 }}>
          ...
          <Typography color="text.secondary">Select an exam above to view your uploaded papers.</Typography>
        </Paper>
      )}
          ...
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            ...
            <Grid item xs={12}>
              <Button variant="outlined" component="label" fullWidth startIcon={<CloudUpload />}
                ...
                {file ? file.name : 'Select PDF File (max 5MB)'}
